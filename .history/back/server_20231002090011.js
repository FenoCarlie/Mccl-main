...
app.post('/login', (req, res) => {
  ...
  // Effectuez une recherche dans la base de données pour vérifier les informations d'identification
  const sql = 'SELECT * FROM user WHERE reg_number = ? AND password = ?';
  db.query(sql, [reg_number, password], (err, results) => {
    if (err) {
    ...
    const token = generateToken(); // Générez un jeton d'accès (vous devrez implémenter cette fonction)
    
    // Vous pouvez également stocker le jeton dans la base de données si nécessaire
