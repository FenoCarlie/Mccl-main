...
app.get("/projet/infoConteneur/:projetId", (req, res) => {
  ...
  const query =
    "SELECT cp.status, c.num_conteneur, c.line, c.type FROM conteneur_projet cp INNER JOIN conteneur c ON cp.conteneur_id = c.id WHERE cp.projet_id = ?";
  ...
  db.query(query, [projetId], (err, result) => {
    if (err) {
      ...
      if (result.length === 0) {
        ...
        res.json(result); // Sélectionnez le premier résultat car il ne devrait y en avoir qu'un
      }
    }
  });
});
