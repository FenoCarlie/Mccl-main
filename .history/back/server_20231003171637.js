...
app.post("/login", (req, res) => {
  ...
  // Récupérez l'utilisateur correspondant au numéro d'enregistrement (registration number)
  const sql = "SELECT * FROM user WHERE reg_number = ?";
  db.query(sql, [reg_number], (err, results) => {
    ...
    bcrypt.compare(password, user.password, (err, isMatch) => {
      ...
      // Les mots de passe correspondent, vous pouvez maintenant générer un jeton JWT si nécessaire

      // Générez et signez le jeton JWT ici
      const token = jwt.sign(
        { reg_number: user.reg_number },
        "votre_secret_jwt",
        { expiresIn: "1h" }
      ); // Remplacez "votre_secret_jwt" par votre propre clé secrète et définissez une expiration appropriée

      // Renvoyez le jeton JWT dans la réponse
...
app.post("/logout", (req, res) => {
  // Récupérez le jeton d'authentification de l'en-tête de la requête
  const userToken = req.headers.authorization;

  // Vérifiez si le jeton existe
  if (!userToken) {
    return res.status(401).json({ error: "Pas de jeton fourni" });
  }

  try {
    // Vérifiez et décodez le jeton
    const decoded = jwt.verify(userToken, "votre_secret_jwt"); // Remplacez "votre_secret_jwt" par votre secret JWT réel

    // Vous pouvez effectuer d'autres opérations de déconnexion ici si nécessaire

    // Envoyez une réponse de succès
    res.status(200).json({ message: "Déconnexion réussie" });
  } catch (error) {
    console.error("Erreur lors de la vérification du jeton :", error);
    res.status(401).json({ error: "Jeton non valide" });
  }
});
