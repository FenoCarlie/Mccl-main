...
app.get('/api/data', (req, res) => {
  ...
  // Utilisez une logique différente en fonction de l'intervalle choisi
  if (interval === 'jour') {
    query = `
      SELECT date, SUM(CASE WHEN category = "export" THEN 1 ELSE 0 END) AS export_count, 
      SUM(CASE WHEN category = "import" THEN 1 ELSE 0 END) AS import_count
      FROM container
    ...
    query = `
      SELECT DATE_FORMAT(date, '%Y-%m') AS date, SUM(CASE WHEN category = "export" THEN 1 ELSE 0 END) AS export_count, 
      SUM(CASE WHEN category = "import" THEN 1 ELSE 0 END) AS import_count
      FROM container
      WHERE date BETWEEN ? AND ?
    ...
    // Logique pour l'intervalle par semaine
    // Vous devrez adapter cette partie en fonction de votre base de données et de la façon dont vous stockez les données par semaine.
    // Cela peut varier en fonction de votre schéma de base de données.
  }
