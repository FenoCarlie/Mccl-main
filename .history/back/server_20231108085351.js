...
app.get("/stockage/getIn/:TpId", (req, res) => {
  ...
  const query = "SELECT * FROM stockage WHERE id = ?";

  db.query(query, [TpId], (err, result) => {
    if (err) {
      console.error(err);
      res.status(500).json({ error: "An error occurred, please try again" });
    } else {
...
app.get("/getIn/:TpId", (req, res) => {
  ...
  WHERE stockage.tp_id = ? AND AND stockage.date_debut IS NULL 
`;
