...
router.get("/user", verifyToken, (req, res) => {
  // L'utilisateur est authentifié, vous pouvez maintenant obtenir les données de l'utilisateur depuis la base de données
  const reg_number = req.user.reg_number; // Vous avez stocké l'identifiant de l'utilisateur dans le token
  ...
  db.query(sql, [reg_number], (err, results) => {
    ...
    const user = results[0];
    res.json(user); // Renvoyez les données de l'utilisateur au client
  });
});
