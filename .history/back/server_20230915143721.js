...
app.delete('/delete/:id_container', (req, res) => {
  ...
  app.get('/get_container/:id_container', (req, res) => {
    const containerId = req.params.id_container;
  
    // Assuming you are using a database, you can use a query to retrieve the container with the specified ID
    const sql = 'SELECT * FROM container WHERE id_container = ?';
    db.query(sql, [containerId], (error, results) => {
      if (error) {
        console.error(error);
        res.sendStatus(500); // Send an error status code back to the client
      } else {
        if (results.length === 0) {
          res.sendStatus(404); // Send a not found status code back to the client if the container is not found
        } else {
          const container = results[0];
          res.json(container); // Send the container data as JSON response back to the client
        }
      }
    });
  });
