...
app.get('/api/data-by-day', (req, res) => {
...
app.get('/api/data-by-month', (req, res) => {
  const startDate = req.query.start;
  const endDate = req.query.end;
  const query = `
...
app.get('/api/data-by-week', (req, res) => {
  const startDate = req.query.start;
  const endDate = req.query.end;
  const query = `
    SELECT YEARWEEK(date) AS week, 
    SUM(CASE WHEN category = "export" THEN 1 ELSE 0 END) AS export_count, 
    SUM(CASE WHEN category = "import" THEN 1 ELSE 0 END) AS import_count
    FROM container
    WHERE date BETWEEN ? AND ?
    GROUP BY week
