      ...
      db.query(insertTransfertSql, insertTransfertValues, (err, result) => {
        ...
        if (lieu_restitution === "Italia") {
          ...
          db.query(insertProjetSql, insertProjetValues, (err, result) => {
            ...
            db.query(insertMouvementProjetSql, insertMouvementProjetValues, (err, result) => {
              if (err) {
                console.error("Erreur lors de l'insertion du mouvement : " + err.message);
                return res.status(500).json({
              ...
              db.query(searchItaliaSql, (err, result) => {
                if (err) {
                  console.error("Erreur lors de la recherche de 'Italia' : " + err.message);
                  return res.status(500).json({
                ...
                const tpId = result[0].id;
                        ...
                        const insertTerrePlainSql = `
                          INSERT INTO terre_plain_mouvement (mouvement_id, tp_id) VALUES (?, ?)
                        `;
        
                        db.query(insertTerrePlainSql, insertTerrePlainValues, (err, result) => {
                          if (err) {
                            console.error("Erreur lors de l'insertion dans terre_plain_mouvement : " + err.message);
                            return res.status(500).json({
                              error: "Erreur lors de l'insertion dans terre_plain_mouvement",
                            });
                          }
