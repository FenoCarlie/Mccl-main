...
app.get("/projet/:clientId", (req, res) => {
  ...
  // Requête SQL pour obtenir toutes les données des tables projet et conteneur_projet liées aux projets du client
  const query = `
    SELECT *
    FROM projet AS p
    LEFT JOIN conteneur_projet AS cp ON p.id = cp.projet_id
    WHERE p.client_projet = ?
  `;
  ...
  db.query(query, [clientId], (err, result) => {
    if (err) {
      ...
      res.json(result); // Renvoyez toutes les données des tables projet et conteneur_projet liées aux projets du client
    }
  });
});
