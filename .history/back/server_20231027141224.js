...
app.post("/create/conteneur/file", (req, res) => {
  const conteneurData = req.body;

  let queries = [];

  for (const conteneur of conteneurData) {
    const { num_conteneur, type, line, tare } = conteneur;
        ...
        db.query(
          ...
          (error, results, fields) => {
            if (error) {
              ...
              if (results.length === 0) {
                // The conteneur does not exist, insert it
                db.query(
                  "INSERT INTO conteneur (num_conteneur, type, line, tare) VALUES (?, ?, ?, ?)",
                  [num_conteneur, type, line, tare],
                  (error, results, fields) => {
                ...
                // The conteneur already exists, skip insertion
                resolve();
              }
            }
          }
