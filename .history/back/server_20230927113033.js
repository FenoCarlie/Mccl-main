...
app.get('/api/data-by-date', (req, res) => {
  const startDate = req.query.start; // Date de début passée en paramètre
  const endDate = req.query.end; // Date de fin passée en paramètre

  const query = `
    SELECT date, SUM(CASE WHEN category = "export" THEN 1 ELSE 0 END) AS export_count, SUM(CASE WHEN category = "import" THEN 1 ELSE 0 END) AS import_count
    FROM container
