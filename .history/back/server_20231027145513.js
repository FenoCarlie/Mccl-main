...
app.post("/create/conteneur/file", upload.single("file"), (req, res) => {
  ...
  for (var R = range.s.r; R <= range.e.r; ++R) {
    ...
    // Utilize the data from the row
    const [num_conteneur, type, line, tare] = row;

    // Check if the container exists already in the database
    queries.push(
        ...
        db.query(
          ...
          (error, results, fields) => {
            if (error) {
              ...
              if (results.length === 0) {
                // Container doesn't exist, insert the row
                db.query(
                ...
                // Container already exists, move to the next row
                resolve();
              }
            }
          }
  ...
  Promise.all(queries)
    .then(() => {
      ...
      // Perform a new SELECT query to get the added containers
    ...
    .catch((error) => {
      // An error occurred during the queries
