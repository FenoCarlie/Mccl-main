...
app.get("/projet/:clientId", (req, res) => {
  ...
  const query = "SELECT * FROM projet WHERE client_projet = ?";
...
app.get("get/projet", (req, res) => {
  ...
  const query =
    "SELECT projet.*, client.* FROM projet LEFT JOIN client ON projet.client_id = client.id ORDER BY projet.date_creation DESC;";
...
app.post("/create/client", (req, res) => {
  ...
  const values = [nom, adresse, contacte];
