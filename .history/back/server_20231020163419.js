...
app.get("/projet/historique/:conteneurId", (req, res) => {
  ...
  const query =
    "SELECT cp.num_projet, p.id AS projet_id, p.nom_projet, p.date_creation, p.num_booking, p.status FROM conteneur_projet cp INNER JOIN projet p ON cp.num_projet = p.id WHERE cp.conteneur_id = ?;";
