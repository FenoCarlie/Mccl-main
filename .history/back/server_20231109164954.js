...
app.get("/stockage/TpPlain/:TpId", (req, res) => {
  ...
  const countConteneursQuery = `
    SELECT COUNT(*) AS total_conteneurs
    FROM conteneur_projet
    WHERE conteneur_projet.etat = 'Plain'
  `;
  ...
  db.query(getStockageQuery, [TpId], (err, result) => {
    ...
    db.query(countConteneursQuery, (countErr, countResult) => {
      if (countErr) {
        console.error(countErr);
        return res.status(500).json({
          error:
            "Une erreur s'est produite lors du comptage des conteneurs, veuillez réessayer",
        });
      }

      const response = {
        stockage: result,
        total_conteneurs: countResult[0].total_conteneurs,
      };
...
app.put("/stockage/GetIn/:containerId", (req, res) => {
  ...
  const { position, date_debut } = req.body;

  const sql = `
    UPDATE stockage
    SET position = ?, date_debut = ?
    WHERE id = ?
  `;

  const values = [position, date_debut, containerId];
