...
app.get("/projet/historique/:conteneurId", (req, res) => {
  ...
  const query =
    "SELECT
    cp.conteneur_id,
    cp.status AS conteneur_status,
    cp.date_ajoute AS date_ajoute_conteneur,
    p.id AS projet_id,
    p.nom_projet,
    c.nom AS nom_client,
    p.date_creation AS date_creation_projet,
    p.num_booking,
    p.status AS projet_status
FROM conteneur_projet cp
INNER JOIN projet p ON cp.projet_id = p.id
INNER JOIN client c ON p.client_projet = c.id
WHERE cp.conteneur_id = ?
ORDER BY date_creation_projet;";
