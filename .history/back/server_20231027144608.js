...
app.post("/create/conteneur/file", upload.single("file"), (req, res) => {
  ...
  for (var R = range.s.r; R <= range.e.r; ++R) {
    ...
    // Utilisez les données de la ligne
    const [num_conteneur, type, line, tare] = row;

    // Vérifiez si le conteneur existe déjà dans la base de données
    queries.push(
      new Promise((resolve, reject) => {
        db.query(
          "SELECT * FROM conteneur WHERE num_conteneur = ?",
          [num_conteneur],
          (error, results, fields) => {
            if (error) {
              console.error(
                "Erreur lors de la vérification du conteneur :",
                error
              );
              reject(error);
            } else {
              if (results.length === 0) {
                // Le conteneur n'existe pas, insérez la ligne
                db.query(
                  ...
                  (error, results, fields) => {
                    if (error) {
                      console.error(
                        "Erreur lors de l'insertion du conteneur :",
                        error
                      );
                      reject(error);
                    } else {
                ...
                // Le conteneur existe déjà, passez à la ligne suivante
                resolve();
              }
            }
          }
  ...
  // Exécutez toutes les requêtes en parallèle
  Promise.all(queries)
    .then(() => {
      // Toutes les requêtes ont été exécutées avec succès
      // Effectuez une nouvelle requête SELECT pour obtenir les conteneurs ajoutés
      db.query("SELECT * FROM conteneur", (error, results, fields) => {
        if (error) {
          console.error(
            "Erreur lors de la récupération des conteneurs :",
            error
          );
          res
            .status(500)
            .send(
              "Une erreur s'est produite lors de la récupération des conteneurs : " +
                error.message
            );
        } else {
    ...
    .catch((error) => {
      // Une erreur s'est produite lors des requêtes
      console.error("Erreur lors de l'ajout des conteneurs :", error);
      res
        .status(500)
        .send(
          "Une erreur s'est produite lors de l'ajout des conteneurs : " +
            error.message
        );
    });
});
