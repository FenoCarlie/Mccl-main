...
app.get("/projet/:clientId", (req, res) => {
  ...
  db.query(projetQuery, [clientId], (err, projetResult) => {
    if (err) {
      ...
      // Requête SQL pour rechercher toutes les lignes de conteneur_projet liées aux projets du client
      ...
      db.query(conteneurQuery, [projetIds], (err, conteneurResult) => {
        if (err) {
          ...
          res.json(conteneurResult); // Renvoyez les données de conteneur_projet
        }
      });
    }
  });
