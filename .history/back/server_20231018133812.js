    ...
    if (err) {
      ...
      db.query(conteneurQuery, [projetIds], (err, conteneurResult) => {
        if (err) {
          ...
          // Comptez le nombre de résultats avec status = true
          const trueCount = conteneurResult.filter(
            (conteneur) => conteneur.status === true
          ).length;
          // Comptez le nombre total de résultats
          const totalCount = conteneurResult.length;
          ...
          res.json({
            projet: projetResult,
            conteneur_projet: conteneurResult,
            ratio: ratio,
          });
        }
      });
