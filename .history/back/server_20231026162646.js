...
app.post("/create/projet", (req, res) => {
  const { nom_projet, client_projet, num_booking, type } = req.body;
  ...
  db.query(insertProjetSql, insertProjetValues, (err, result) => {
    ...
    const insertProjetTypeValues = [type, projet_id];
...
app.post("/create/conteneur/file", upload.single("file"), (req, res) => {
  for (var R = range.s.r; R <= range.e.r; ++R) {
    ...
    // Only insert the row if num_conteneur is not empty and doesn't exist in the database
    if (num_conteneur) {
      queries.push(
          ...
          db.query(
            ...
            (error, results, fields) => {
              if (error) {
                ...
                if (results.length === 0) {
                  // The conteneur doesn't exist, insert the row
                  db.query(
                  ...
                  // The conteneur already exists, skip the insertion
                  resolve();
                }
              }
            }
  ...
  Promise.all(queries)
    .then(() => {
      ...
      res.status(200).send("Conteneurs ajoutés avec succès !");
    })
    .catch((error) => {
      // Error occurred during queries
      res
        .status(500)
        .send(
