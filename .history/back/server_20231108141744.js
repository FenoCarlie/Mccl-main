...
app.get("/projet/:clientId", (req, res) => {
  ...
  db.query(projetQuery, [clientId], (err, projetResult) => {
    if (err) {
      ...
      // Créez un tableau pour stocker les détails de chaque projet
      const projetsAvecDetails = projetResult.map((projet) => {
        // Requête SQL pour obtenir toutes les données de la table conteneur_projet liées au projet
        const conteneurQuery =
          "SELECT * FROM conteneur_projet WHERE projet_id = ?";
        db.query(conteneurQuery, [projet.id], (err, conteneurResult) => {
          if (err) {
            console.error(err);
            res
              .status(500)
            ...
            // Créez un objet projetDetails avec les valeurs calculées
            const projetDetails = {
              totalConteneurs,
              conteneursActifs,
              conteneursTermines,
            ...
            // Retournez le projet avec les détails inclus
            return {
              ...projet,
              projetDetails,
            };
