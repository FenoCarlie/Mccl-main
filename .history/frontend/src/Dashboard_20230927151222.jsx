...
// Fonction pour formater la date en "YYYY-MM" (par mois)
...
function Dashboard() {
  ...
  const [startDate, setStartDate] = useState(formatDateToYYYYMMDD(new Date())); // Date de début par défaut
  const [endDate, setEndDate] = useState(formatDateToYYYYMMDD(new Date())); // Date de fin par défaut
  ...
  const fetchData = () => {
    ...
    let query = '';
    if (interval === 'jour') {
      query = `http://localhost:8081/api/data-by-date?start=${formattedStartDate}&end=${formattedEndDate}`;
    } else if (interval === 'mois') {
      query = `http://localhost:8081/api/data-by-month?start=${formattedStartDate}&end=${formattedEndDate}`;
    } else if (interval === 'semaine') {
      query = `http://localhost:8081/api/data-by-week?start=${formattedStartDate}&end=${formattedEndDate}`;
    }
  ...
  const formatXAxisDate = (date) => {
    ...
    if (interval === 'jour') {
      ...
      return formatDateToYYYYMM(formattedDate);
    } else if (interval === 'semaine') {
      // Formatez les dates pour l'intervalle par semaine selon vos besoins
      // Par exemple, vous pouvez utiliser une logique pour afficher le numéro de semaine.
      return 'Semaine X';
    }
  };
