...
export default function InfoProjet() {
  ...
  const getProjet = (projetId) => {
    axios
      ...
      .then(({ data }) => {
        setLoading(false);
        setProjet(data[0]);
        if (data[0].type === "Export" || data[0].type === "Import") {
          setEtat("Plain");
        }
      })
      .catch((error) => {
  ...
  const ajoutProjet = (e) => {
    ...
    // Extract the container IDs from the selectedContainers state
    const containerIds = selectedContainers.map((container) => container.id);

    // Make a POST request to the server with the container IDs
    axios
      .post("http://localhost:8081/projet/ajoutProjet", {
        projetId,
        containerIds,
      })
      .then((response) => {
        ...
        setNotification("Containers added successfully");
        getConteneur(projetId);
        setIsModalConteneurOpen(false);
      })
      .catch((error) => {
        // Handle any errors that occur during the request
        console.error("An error occurred while adding containers:", error);
                  ...
                  <label className="">
                    {conteneur.status === 1 ? (
                      pathWithoutLastSegment === "/projet/infoProjet" ? (
                        <Link
                          to={`/projet/infoProjet/infoConteneur/${conteneur.id}/${projetId}`}
                        >
                          Terminer
                        </Link>
                      ) : (
                        <Link
                          to={`/client/infoClient/infoProjet/infoConteneur/${conteneur.id}/${projetId}`}
                        >
                          Terminer
                        </Link>
                      )
