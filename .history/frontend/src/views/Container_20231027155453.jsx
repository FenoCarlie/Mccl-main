...
export default function Container() {
  ...
  const onSubmit = (ev) => {
    ...
    reader.onload = (e) => {
      ...
      const range = "D3:G300"; // Specify the range of columns to read
      const columnNamesRange = "D3:G3"; // Specify the range of column names
      const columnsToRead = utils.sheet_to_json(worksheet, { range });
      const columnNames = utils.sheet_to_json(worksheet, {
        range: columnNamesRange,
      });

      // Use columnsToRead and columnNames as needed
      console.log(columnsToRead);
      console.log(columnNames);

      // Append the converted JSON data to the formData
      formData.append("jsonData", JSON.stringify(columnsToRead));

      axios
        .post("http://localhost:8081/create/conteneur/", formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
        .then(() => {
          setNotification("Le conteneur a été créé avec succès");
          setLoading(true);
          setErrors({});
          ...
          >
            <div className="align">
              <form className="form upload" onSubmit={onSubmit}>
                <span className="form-title">Chargez votre fichier</span>
                <p className="form-paragraph">Le fichier doit être un exel</p>
                <label className="drop-container">
                  <span className="drop-title">Déposez vos fichiers ici</span>
                  ou
                  <input
                    type="file"
                    ref={fileInput}
                ...
                <button className="btn">Sauvegarde</button>
              </form>
              <div>
