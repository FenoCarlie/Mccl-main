...
export default function Container() {
  ...
  const onSubmit = (ev) => {
    ...
    var workbook = XLSX.readFile(fileInput.current.files[0]);
    ...
    for (var R = range.s.r; R <= range.e.r; ++R) {
      ...
      // Only add the row if num_conteneur is not empty
      if (num_conteneur) {
        conteneurData.push({ num_conteneur, type, line, tare });
      }
    }

    axios
      .post("http://localhost:8081/create/conteneur/file", conteneurData)
      .then(() => {
