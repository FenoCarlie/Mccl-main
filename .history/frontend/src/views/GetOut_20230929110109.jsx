...
const getContainer = () => {
  ...
  .then((response) => {
    console.log(response); // Affichez la réponse du serveur ici
  ...
  .then(() => {
    ...
    navigate('/container/getOut');
    setErrors({});
    updateTableData(container); // Assurez-vous que cette ligne est bien présente
  })
  .catch((err) => {
    ...
    <>
      ...
      <div className="align">
        ...
        <div className="card card_width">
          ...
          <DataTable columns={columnsToDisplay} data={tableData} customStyles={customStyles} />
        </div>
      </div>
    </>
  );
