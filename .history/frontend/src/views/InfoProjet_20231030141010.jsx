...
export default function InfoProjet() {
  ...
  const onSubmit = (ev) => {
    ...
    const selectedContainers = selectedContainers.map((container) => ({
      conteneur_id: container.id,
      projet_id: projetId,
    }));

    axios
      .post("http://localhost:8081/create/conteneur_projet", {
        selectedContainers,
      })
      .then(() => {
        setNotification("Les conteneurs ont été ajoutés avec succès");
        setLoading(true);
        setErrors({});
