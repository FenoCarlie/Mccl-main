...
export default function InfoProjet() {
  const { selectedProjetId } = useParams();
  ...
  const getProjet = (selectedProjetId) => {
    axios
      .get(`http://localhost:8081/projet/infoProjet/${selectedProjetId}`)
      .then(({ data }) => {
        setProjet(data);
        setLoading(false);
      })
  ...
  const getConteneur = (selectedProjetId) => {
    axios
      .get(`http://localhost:8081/projet/infoConteneur/${selectedProjetId}`)
      .then(({ data }) => {
        selectedProjetId;
        setConteneurs(data);
      })
  ...
  useEffect(() => {
    getProjet(selectedProjetId);
    getConteneur(selectedProjetId);
  }, [selectedProjetId]);
