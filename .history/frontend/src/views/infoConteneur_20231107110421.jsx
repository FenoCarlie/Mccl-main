...
export default function InfoConteneur() {
  ...
  const handleSelectReastitution = (ev) => {
    ...
    if (ev.target.value !== "Autre...") {
      const newProjectName = generateNewProjectName();
      setMouvements({
        ...mouvements,
        lieu_restitution: ev.target.value,
        nom_projet: newProjectName,
      });
    }
  };
  ...
  const generateNewProjectName = () => {
    if (type === "Italia") {
      return "STC" + conteneur.nom_projet.substring(3);
    }
    return conteneur.nom_projet;
  };
  
  useEffect(() => {
    const newProjectName = generateNewProjectName();
    setNomProjetStc(newProjectName);
  }, [type, conteneur.nom_projet]);
  ...
  const insertNewSt = {
    conteneur_id: conteneurId,
    client_id: conteneur.client_projet,
  }
