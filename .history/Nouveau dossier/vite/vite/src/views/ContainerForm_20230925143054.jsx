  ...
  export default function ContainerForm() {
    ...
    // État pour gérer la saisie de l'utilisateur et les suggestions de clients
  const [clientInput, setClientInput] = useState("");
  const [clientSuggestions, setClientSuggestions] = useState([]);
  const [selectedClientId, setSelectedClientId] = useState("");

  // Fonction pour rechercher les suggestions de clients
  const searchClientSuggestions = (inputValue) => {
    // Faites une requête à votre API pour obtenir les suggestions de clients
    axios
      .get(`http://localhost:8081/clients?q=${inputValue}`)
      .then(({ data }) => {
  ...
  // Gérer la saisie de l'utilisateur dans le champ "client"
  const handleClientInputChange = (ev) => {
    const inputValue = ev.target.value;
    setClientInput(inputValue);

    // Recherchez des suggestions chaque fois que l'utilisateur tape
    searchClientSuggestions(inputValue);
  };

  // Gérer la sélection d'un client depuis les suggestions
  const handleClientSelect = (selectedClient) => {
    setClientInput(selectedClient.name); // Affichez le nom du client dans le champ
    setSelectedClientId(selectedClient.id); // Utilisez l'ID du client comme valeur

    // Effacez les suggestions après la sélection
    ...
    return (
          ...
          <div className="card card_width animated fadeInDown">
            <div className="test">
                ...
                <div className="align">
                  ...
                  <select
                    className="input-field"
                    value={line} // Utilisez la valeur de 'line' ici
                    onChange={handleSelectLine}
                    id="line" // Ajoutez un id unique
                  >
      ...
      {/* Affichez les suggestions de clients */}
