...
export default function EditContainer() {
  ...
  const validateForm = () => {
    ...
    if (!container.num_container) {
      validationErrors.num_container = 'Number of container is required';
    }

    // Perform additional validation checks and update `validationErrors` object
  ...
  const onSubmit = (ev) => {
    ...
    if (validateForm()) {
      axios
        .put(`http://localhost:8081/container/${container.id_container}`, container)
        .then(() => {
  ...
  return (
    <div>
      ...
      {console.log (container.id_container)}
